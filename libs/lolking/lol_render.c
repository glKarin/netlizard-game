...
void LOL_UpdateVertex(const LOL_Mesh *mesh, GLfloat *verteces)
{
	...
	for(i = 0; i < mesh->vertex_count; i++)
	{
		...
		const vector3_s v = {vertex->position[0], vertex->position[1], vertex->position[2]};
		const vector3_s n = {vertex->normal[0], vertex->normal[1], vertex->normal[2]};
		vector3_s nv = {0.0, 0.0, 0.0};
		vector3_s nn = {0.0, 0.0, 0.0};
		unsigned int j;
		...
		for(j = 0; j < len; j++)
		{
			...
			vector3_s vv = Math3D_Vector3MultiplyMatrix44(&v, &mat);
			vector3_s vn = Math3D_Vector3MultiplyMatrix44(&n, &mat);
