...
float Physics_GetGravityPositionH(float speed, const nl_vector3_s *start_pos, const nl_vector3_s *last_pos, float x_dir, float y_dir, long long time, int gravity, nl_vector3_s *new_pos, float *rx, float *ry)
{
	if(!start_pos || !last_pos)
		return -1;
	nl_vector3_s dir = Algo_ComputeDirection(y_dir, x_dir);
	...
	float yr = y_dir;
	float dis = 0.0;
	nl_vector3_s v = Vector3_SubtractVector3(&pos, last_pos);
	dis = Vector3_Mag(&v);
	if(gravity)
	{
...
float Physics_GetGravityPosition(float speed, const nl_vector3_s *start_pos, const nl_vector3_s *last_pos, float x_dir, float y_dir, long long time, int gravity, nl_vector3_s *new_pos, float *rx, float *ry)
{
	if(!start_pos || !last_pos)
		return -1;
	float xr = x_dir;
	...
	if(gravity)
	{
		...
		nl_vector3_s xdir = Algo_ComputeDirection(y_dir, 0.0);
		xdir = Vector3_Scale(&xdir, xdis);

		float ydis = -yspeed * delta_time + -GAME_G * delta_time * delta_time * 0.5;
		nl_vector3_s ydir = {0.0, 0.0, 1.0};
		ydir = Vector3_Scale(&ydir, ydis);
		nl_vector3_s dir = Vector3_PlusVector3(&xdir, &ydir);
		pos = Vector3_PlusVector3(start_pos, &dir);
		v = Vector3_SubtractVector3(&pos, last_pos);
		dis = Vector3_Mag(&v);
		Vector3_Normalize(&v);
