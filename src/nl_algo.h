...
// 计算当前坐标与场景地板的交点
int Algo_ComputeFloorCoord(const GL_NETLizard_3D_Model *netlizard_3d_model, const nl_vector3_t *v, int *scene, float *rglz);
// 地图场景中碰撞测试
// 返回结果场景 -1 表示不在任何场景
// 返回0 不在任何场景 1 只在AABB盒子内 2 不仅在AABB盒子内，还在各个场景碰撞面内
map_collision_testing_result_type Algo_NETLizard3DMapCollisionTesting(const GL_NETLizard_3D_Model *netlizard_3d_model, const nl_vector3_t *n_xyz, float width, float height, int *scene);
// 场景中物品的碰撞测试
item_collision_testing_result_type Algo_NETLizard3DItemCollisionTesting(const GL_NETLizard_3D_Model *netlizard_3d_model, const nl_vector3_t *xyz, const nl_vector3_t *n_xyz, float width, float height, int scene, nl_vector3_t *c_normal, int *index);
// 获取物品的顶面坐标
int Algo_GetItemTopCoord(const GL_NETLizard_3D_Model *netlizard_3d_model, const nl_vector3_t *n_xyz, int scene, float width, float *rglz, int *index, float *height);
// 获取物品的顶面坐标，无场景的情况下
int Algo_GetItemTopCoordForAll(const GL_NETLizard_3D_Model *netlizard_3d_model, const nl_vector3_t *n_xyz, float *rglz, int *index);
// 点是否在场景盒子内
int Algo_PointInAABBInNETLizard3DMap(const nl_vector3_t *p, const GL_NETLizard_3D_Model *map_model, GLuint scene);
...
// 射线碰撞测试 绘制准星红点
int Algo_RayCollisionTestingInNETLizard3DMap(const GL_NETLizard_3D_Model *netlizard_3d_model, const nl_vector3_t *pos, const nl_vector3_t *dir, float width, int *scene, float *distance, nl_vector3_t *c_point, nl_vector3_t *c_normal);
// 两点之间是否彼此可见，模拟射击是否击中
