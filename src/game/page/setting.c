...
static int Menu_SettingMouseClickEventFunc(int b, int x, int y);
static int Menu_SettingMouseMotionEventFunc(int b, int p, int x, int y, int dx, int dy);
static void Menu_SettingDrawFunc(void);
static void Menu_SettingReshapeFunc(int w, int h);
static void Menu_SettingInitFunc(void);
static void Menu_SettingFreeFunc(void);
...
int Menu_SettingIdleEventFunc(void)
{
	if(!has_init)
		return 0;
...
void Menu_SettingInitFunc(void)
{
...
void Menu_SettingDrawFunc(void)
{
	if(!has_init)
		return;
...
void Menu_SettingReshapeFunc(int w, int h)
{
...
void Menu_SettingFreeFunc(void)
{
	if(!has_init)
		return;
...
int Menu_SettingKeyEventFunc(int key, int act, int pressed, int x, int y)
{
	if(!has_init)
		return 0;
	switch(key)
...
int Menu_SettingMouseEventFunc(int button, int pressed, int x, int y)
{
	if(!has_init)
		return 0;
	int gl_y = height - y;
...
int Menu_SettingMouseMotionEventFunc(int button, int pressed, int x, int y, int dx, int dy)
{
	if(!has_init)
		return 0;
	int res = 0;
...
void Menu_SettingRegisterFunction(void)
{
	Main3D_SetInitFunction(Menu_SettingInitFunc);
	Main3D_SetDrawFunction(Menu_SettingDrawFunc);
	Main3D_SetFreeFunction(Menu_SettingFreeFunc);
	Main3D_SetKeyEventFunction(Menu_SettingKeyEventFunc);
	Main3D_SetIdleEventFunction(Menu_SettingIdleEventFunc);
	Main3D_SetReshapeFunction(Menu_SettingReshapeFunc);
	Main3D_SetMouseEventFunction(Menu_SettingMouseEventFunc);
	Main3D_SetMouseMotionEventFunction(Menu_SettingMouseMotionEventFunc);
	Main3D_SetMouseClickEventFunction(Menu_SettingMouseClickEventFunc);
}
...
int Menu_SettingMouseClickEventFunc(int button, int x, int y)
{
	if(!has_init)
		return 0;
	int gl_y = height - y;
